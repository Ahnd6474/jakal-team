...
const PROJECTS = [
  ...
  },
  {
    ...
    shortDesc: {
      ko: 'AI 기반 토마토 식물 질병 진단 시스템',
      en: 'AI-powered tomato plant disease diagnosis system'
    },
    fullDesc: {
      ko: '딥러닝 이미지 분류 모델을 활용해 토마토 잎 사진에서 질병을 자동으로 진단하는 시스템입니다. 농업 현장에서 육안 검사를 대체하고, 조기 질병 탐지를 통해 작물 손실을 최소화하는 것을 목표로 합니다. CNN 기반 분류 모델을 학습시켜 여러 가지 병해 유형을 구분합니다.',
      en: 'A system that automatically diagnoses diseases from photos of tomato leaves using deep learning image classification. It aims to replace visual inspection in agricultural fields and minimize crop loss through early disease detection. A CNN-based classification model is trained to distinguish multiple disease types.'
    },
  ...
  },
  {
    ...
    title: 'ColonyPlate',
    shortDesc: {
      ko: '배양 플레이트 콜로니 자동 계수 시스템',
      en: 'Automated colony counting system for culture plates'
    },
    fullDesc: {
      ko: '실험실에서 수작업으로 진행하던 세균 콜로니 카운팅을 자동화한 컴퓨터 비전 프로젝트입니다. 배양 플레이트 이미지를 분석하여 콜로니 수를 자동으로 계수함으로써 실험 시간과 오차를 줄입니다. 생명과학 분야의 반복 작업 자동화 사례입니다.',
      en: 'A computer vision project that automates bacterial colony counting, which was previously done by hand in laboratories. By analyzing culture plate images and automatically counting colonies, it reduces experiment time and human error — a showcase of automation in life science.'
    },
  ...
  // ---- Made with Jakal-flow ----
  {
    ...
    shortDesc: {
      ko: 'Jakal-flow + Gemini 3 Flash로 생성한 웹사이트',
      en: 'Website generated with Jakal-flow + Gemini 3 Flash'
    },
    fullDesc: {
      ko: 'Jakal-flow 시스템을 사용해 Google Gemini 3 Flash 모델이 자동으로 구성하고 생성한 웹사이트 프로젝트입니다. AI가 콘텐츠 기획부터 코드 생성까지 전 과정을 수행하며, Jakal-flow의 멀티 에이전트 파이프라인이 각 단계를 조율합니다.',
      en: 'A website project automatically structured and generated by Google Gemini 3 Flash using the Jakal-flow system. The AI handles everything from content planning to code generation, with Jakal-flow\'s multi-agent pipeline coordinating each step.'
    },
